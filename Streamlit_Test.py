...
#=====================================================================
def Is_tab_visible():
    """Return True if the browser tab is visible, False otherwise.

    Use as:
    @st.fragment(run_every=4)
    def Page1_fragment():
        if Is_tab_visible():
            st.write("Fragment draait…")
        else:
            st.write("Tab niet zichtbaar → fragment pauzeert.")
    ...
    # JavaScript to detect visibility changes
    js_code = """
    <script>
    document.addEventListener('visibilitychange', function() {
        const isVisible = !document.hidden;
        window.parent.postMessage({isVisible: isVisible}, "*");
    });
    </script>
    """
    ...
    # Listener to receive messages from JS
    listener = """
    <script>
    window.addEventListener("message", (event) => {
        if (event.data && event.data.isVisible !== undefined) {
            window.parent.postMessage(
                {streamlitMessage: {tab_visible: event.data.isVisible}},
                "*"
            );
        }
    });
    ...
    """
    st.markdown(listener, unsafe_allow_html=True)

    # Update value from JS events
    if "streamlitMessage" in st.session_state:
        st.session_state.tab_visible = st.session_state.streamlitMessage["tab_visible"]

    return st.session_state.tab_visible
#=====================================================================
